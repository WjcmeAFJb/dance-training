// Step verifier. Runs after every dispatched command.

import type { EditorState, Range } from "../emulator/types.ts";
import { rangeEqual } from "../emulator/textOps.ts";
import type { Goal } from "./types.ts";

export function verify(state: EditorState, goal: Goal): boolean {
  switch (goal.kind) {
    case "text-equals":
